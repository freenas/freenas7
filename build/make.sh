...
FREENAS_SVNURL="https://freenas.svn.sourceforge.net/svnroot/freenas/branches/0.686"
	...
	for choice in $(echo $choices | tr -d '"'); do
		case $choice in
			...
			install)
				...
				cp -v -p ./apm/apm.ko $FREENAS_ROOTFS/boot/kernel;
				cp -v -p ./acpi/acpi/acpi.ko $FREENAS_ROOTFS/boot/kernel;
